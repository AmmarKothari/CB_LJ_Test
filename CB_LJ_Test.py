...
def init_setup():
    ...
    try:
        ...
        sys.exit()
        #LJ1.open()
    LJ1_ID = LJ1.id #labjack id should be zero
    if LJ1_ID != 0:
...
def SevenFiveVCheck():
    ...
    AverageReadVoltage = MeasureVoltage(SevenFivePin) #measure actual voltage through divider
    AverageAdjustedVoltage = AverageReadVoltage / R_S_Ratio #Adjust by divider to find CB output.
    Result = VoltageAcceptable(AverageAdjustedVoltage, 7, 8) #Compare to acceptable values
    logging.debug("Adjusted Volgate: %s" %AverageAdjustedVoltage)
    return(AverageAdjustedVoltage, Result)

def TwentyFourLowVCheck():
    global TwentyFourPin, R_TF_Ratio
    logging.debug("24V Low Voltage Check")
    AverageReadVoltage = MeasureVoltage(TwentyFourPin)
    AverageAdjustedVoltage = AverageReadVoltage / R_TF_Ratio #Voltage read with initialized divider. Adjust to find CB output.
    Result = VoltageAcceptable(AverageAdjustedVoltage, -.1, .1)
    logging.debug("Adjusted Volgate: %s" %AverageAdjustedVoltage)
    return(AverageAdjustedVoltage, Result)
...
def test_Sequence():
    ...
    #Reset labjack
    #Reset_LabJack()
    #is a calibration check necessary???
    #Check for 7.5V
    SevenFiveVCheck()
